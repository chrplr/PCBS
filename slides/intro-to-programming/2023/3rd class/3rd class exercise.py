...
# solution 2 a bit more complex that goes from 00000 to 99999
lottery_list=[]

for i in range(100): # We do 100 time the ticket selection
    ticket = []
    for j in range(0,5): # We select 5 numbers randomly from 0 to 9
        ticket.append(randrange(0,9)) 
    lottery_list.append(''.join([str(item) for item in ticket])) # Quite complex code to transform every number (list of 5 digit) into one 5 digit number
print(lottery_list)
winner = sample(lottery_list,1)
print("Winner ticket: ", winner)
